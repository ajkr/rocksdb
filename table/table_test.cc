...
TEST_F(BlockBasedTableTest, SplitRangeDelCoveringPut) {
  ...
  const InternalKey kTableLowerBound(
      kBeginKey, kMaxSequenceNumber, kTypeRangeDeletion);
  ...
  range_del_agg.AddTombstones(
      std::move(range_del_iter), &kTableLowerBound, &kTableUpperBound);
  ...
  std::unique_ptr<InternalIterator> iter(
      reader->NewIterator(ReadOptions(), &range_del_agg, &meta));
  iter->SeekToFirst();
  ASSERT_FALSE(iter->Valid());
