...
struct CompactionJob::SubcompactionState {
  ...
  // it'll reach the maximum length. `CompressionOptions::zstd_max_train_bytes`
  // or `CompressionOptions::max_dict_bytes` control the amount of sampling, and
  // the samples are appended to `compression_dict_samples`.
  //
  // After the first file is generated, the final dictionary is generated by
  // zstd's dictionary trainer if `CompressionOptions::zstd_max_train_bytes` was
  // nonzero; otherwise, the samples are used directly. The final dictionary is
  // stored in `compression_dict` and applied to subsequent output files in the
  // same subcompaction.
  ...
  SubcompactionState& operator=(SubcompactionState&& o) {
    ...
    compression_dict_samples = std::move(o.compression_dict_samples);
    return *this;
  }
  ...
  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
    ...
    if (output_file_ended) {
      ...
      if (sub_compact->outputs.size() == 1) {
        ...
        // compression of subsequent files. OK if compression_dict_samples is
        // empty, we'll just get an empty dictionary.
        if (cfd->ioptions()->compression_opts.zstd_max_train_bytes > 0) {
          sub_compact->compression_dict = ZSTD_TrainDictionary(
              sub_compact->compression_dict_samples, 6 /* TODO fix */,
              cfd->ioptions()->compression_opts.max_dict_bytes);
        } else {
          sub_compact->compression_dict =
              std::move(sub_compact->compression_dict_samples);
        }
      }
    }
  }
...
Status CompactionJob::OpenCompactionOutputFile(
  ...
  // Sample the first output file to generate dictionary that'll be applied
  // on subsequent output files. Currently we only incur this overhead for files
  // in the bottommost level as they account for the majority of the data.
  std::string* compression_dict_samples;
  if (bottommost_level_ && sub_compact->outputs.size() == 1) {
    compression_dict_samples = &sub_compact->compression_dict_samples;
  } else {
    compression_dict_samples = nullptr;
  }
  ...
  sub_compact->builder.reset(NewTableBuilder(
      *cfd->ioptions(), *sub_compact->compaction->mutable_cf_options(),
      ...
      skip_filters, output_file_creation_time, compression_dict_samples));
  LogFlush(db_options_.info_log);
  return s;
}
