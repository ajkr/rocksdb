...
DEFINE_string(
    ...
    "fillseqtimestamphash,"
    "fillsync,"
    "fillrandom,"
    "filluniquerandomdeterministic,"
    "overwrite,"
    "readrandom,"
    "readrandomtimestamphash,"
    "newiterator,"
    "newiteratorwhilewriting,"
    "seekrandom,"
    "seekrandomtimestamphash,"
    ...
    "\tfillseqtimestamphash       -- write N values in the specified"
    " timestamp order where the key is a hash of the timestamp\n"
    ...
    "\treadrandomtimestamphash    -- read N times in random order using exact "
    "timestamp and key generated by hashing that timestamp\n"
...
class Benchmark {
 private:
  ...
  bool use_timestamp_hash_as_key_;
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
...
#endif  // !ROCKSDB_LITE
        use_timestamp_hash_as_key_(false) {
  ...
  void GenerateKeyFromInt(uint64_t v, int64_t num_keys, Slice* key) {
    ...
    if (use_timestamp_hash_as_key_) {
      // Our fork of xxh3p hashes the seed when input is empty string so this
      // should have good spread.
      key_number = Hash64("" /* data */, 0 /* length */, v /* seed */);
    } else {
      key_number = v;
    }
    ...
    if (port::kLittleEndian) {
      for (int i = 0; i < bytes_to_fill; ++i) {
        pos[i] = (key_number >> ((bytes_to_fill - i - 1) << 3)) & 0xFF;
      }
    } else {
      memcpy(pos, static_cast<void*>(&key_number), bytes_to_fill);
    }
    pos += bytes_to_fill;
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (name == "fillseqdeterministic" ||
        ...
        method = &Benchmark::WriteSeq;
      } else if (name == "fillseqtimestamphash") {
        fresh_db = true;
        use_timestamp_hash_as_key_ = true;
        method = &Benchmark::WriteSeq;
      ...
      } else if (name == "readrandomtimestamphash") {
        use_timestamp_hash_as_key_ = true;
        method = &Benchmark::ReadRandom;
      ...
      } else if (name == "seekrandomtimestamphash") {
        use_timestamp_hash_as_key_ = true;
        method = &Benchmark::SeekRandom;
 ...
 private:
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    std::vector<Slice> ts_list;
    std::vector<std::unique_ptr<char[]>> ts_guards;
    if (user_timestamp_size_ > 0) {
      size_t num_user_ts_per_batch;
      if (use_timestamp_hash_as_key_) {
        num_user_ts_per_batch = static_cast<size_t>(entries_per_batch_);
      } else {
        num_user_ts_per_batch = 1;
      }
      ts_guards.resize(num_user_ts_per_batch);
      for (size_t j = 0; j < static_cast<size_t>(entries_per_batch_); ++j) {
        ts_guards[j].reset(new char[user_timestamp_size_]);
      }
      ts_list.resize(num_user_ts_per_batch);
    }
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      for (int64_t j = 0; j < entries_per_batch_; j++) {
        ...
        if (user_timestamp_size_ > 0 && use_timestamp_hash_as_key_) {
          // This is only supported in sequential fill, so the clock should
          // stay in sync with the key number.
          assert(mock_app_clock_->Get() == rand_num);
          ts_list[j] = mock_app_clock_->Allocate(ts_guards[j].get());
        }
      }
      ...
      if (user_timestamp_size_ > 0) {
        if (use_timestamp_hash_as_key_) {
          s = batch.AssignTimestamps(ts_list);
        } else {
          ts_list[0] = mock_app_clock_->Allocate(ts_guards[0].get());
          s = batch.AssignTimestamp(ts_list[0]);
        }
        if (!s.ok()) {
...
// optimization, so skip the check of the max level.
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      if (user_timestamp_size_ > 0) {
        if (use_timestamp_hash_as_key_) {
          EncodeFixed64(ts_guard.get(), key_rand);
          ts = Slice(ts_guard.get(), FLAGS_user_timestamp_size);
        } else {
          ts = mock_app_clock_->GetTimestampForRead(thread->rand,
                                                    ts_guard.get());
        }
  ...
  void SeekRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      if (user_timestamp_size_ > 0) {
        if (use_timestamp_hash_as_key_) {
          EncodeFixed64(ts_guard.get(), seek_pos);
          ts = Slice(ts_guard.get(), FLAGS_user_timestamp_size);
        } else {
          ts = mock_app_clock_->GetTimestampForRead(thread->rand,
                                                    ts_guard.get());
